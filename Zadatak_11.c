...
int IspisTablice(Student **Table, int b);
int IspisListe(Student *Element, int i);
int IspisMaticnogBroja(Student *Element);
Student *PronadiElement(Student **Table, char *ime, char *prezime);
...
Student *PronadiElement(Student **Table, char *ime, char *prezime) {
	...
	x = IzracunTablice(prezime, SIZE);
	Student *Element = Table[x]->next;

	while (Element)
		if (!strcmp(Element->prezime, prezime) && !strcmp(Element->ime, ime))
			return Element;
		else
			Element = Element->next;
